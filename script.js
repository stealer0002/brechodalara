...
function applyFilters() {
    ...
    if (activeSize) {
        ...
        // Correção: Se o botão de tamanho tiver um grupo específico (ex: calçados), força a categoria
        if (activeSize.dataset.categoryGroup) {
            filtered = filtered.filter(p => p.category === activeSize.dataset.categoryGroup);
        }
    }
